...
<div class="max-w-7xl mx-auto p-4">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold">Assistance Requests</h1>
        <form method="GET" class="flex gap-2">
            <input type="text" name="q" value="{{ request('q') }}" class="form-control" placeholder="Search requests or users...">
            ...
            <table class="table table-report -mt-2 w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $r->constituent?->first_name }} {{ $r->constituent?->last_name }}</td>
                        <td>{{ $r->office?->name ?? '—' }}</td>
                        ...
                        <td class="text-center">
                            <div class="flex justify-center items-center gap-2">
                                @if($r->status==='pending')
                                <form method="POST" action="{{ route('requests.approve', $r->id) }}">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('requests.disburse', $r->id) }}">
                                    @csrf
                    ...
                    @empty
                    <tr><td colspan="7" class="text-center py-10 text-slate-500">No requests found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
