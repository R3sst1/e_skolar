...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update progress when files are selected
        ...
        input.addEventListener('change', function() {
            ...
            if (file) {
                ...
                if (file.size > 2 * 1024 * 1024) {
                    statusDiv.innerHTML = '<span class="text-danger text-xs">File too large (max 2MB)</span>';
                    this.value = '';
                    return;
                }
                ...
                if (!allowedTypes.includes(file.type)) {
                    statusDiv.innerHTML = '<span class="text-danger text-xs">Invalid file type</span>';
                    this.value = '';
                    return;
                }

                statusDiv.innerHTML = '<span class="text-success text-xs">✓ ' + file.name + ' (' + (file.size / 1024).toFixed(1) + ' KB)</span>';
                uploadedCount++;
            } else {
                statusDiv.innerHTML = '';
                uploadedCount--;
            ...
            // Update progress
            const progress = (uploadedCount / documentInputs.length) * 100;
            progressBar.style.width = progress + '%';
            uploadProgress.textContent = uploadedCount + '/' + documentInputs.length;

            // Enable/disable submit button
            if (uploadedCount === documentInputs.length) {
