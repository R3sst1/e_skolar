                    ...
                    <div class="col-span-12">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ Storage::url($application->grade_photo) }}" 
                                   target="_blank" 
...
<!-- Grade Photo Preview Modal -->
<div id="grade-photo-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Grade Photo Preview</h2>
            ...
            <div class="modal-body">
                <div class="text-center">
                    <img src="{{ $application->grade_photo ? asset('storage/' . $application->grade_photo) : '' }}" 
                         alt="Grade Photo" 
                         class="max-w-full max-h-96 mx-auto rounded-lg shadow-lg cursor-pointer"
                         id="grade-photo-preview"
                         onclick="toggleFullscreen()">
                </div>
                <div class="text-center mt-4">
                    <button id="fullscreen-grade-btn" class="btn btn-outline-primary mr-2" title="View Fullscreen">
                        <i data-lucide="maximize" class="w-4 h-4 mr-2"></i>
                        Fullscreen
                    </button>
                    <a href="{{ $application->grade_photo ? asset('storage/' . $application->grade_photo) : '#' }}" 
                       download 
...
<!-- Fullscreen Grade Photo Modal -->
<div id="fullscreen-grade-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-full">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Grade Photo - Fullscreen View</h2>
                <button data-tw-dismiss="modal" class="btn btn-outline-secondary hidden sm:flex">
                    <i data-lucide="x" class="w-4 h-4"></i>
                </button>
            </div>
            <div class="modal-body p-0">
                <div class="flex items-center justify-center h-full min-h-96">
                    <img src="{{ $application->grade_photo ? asset('storage/' . $application->grade_photo) : '' }}" 
                         alt="Grade Photo Fullscreen" 
                         class="max-w-full max-h-full object-contain"
                         id="fullscreen-grade-image">
                </div>
            </div>
            <div class="modal-footer">
                <a href="{{ $application->grade_photo ? asset('storage/' . $application->grade_photo) : '#' }}" 
...
// Grade photo preview function
function previewGradePhoto() {
    // Handle tailwind.Modal safely
    if (typeof tailwind !== 'undefined' && tailwind.Modal) {
        const modal = new tailwind.Modal(document.getElementById('grade-photo-modal'));
        modal.show();
    } else {
        // Fallback: show modal directly
        document.getElementById('grade-photo-modal').style.display = 'block';
    }
}
...
function toggleFullscreen() {
    // Handle tailwind.Modal safely
    if (typeof tailwind !== 'undefined' && tailwind.Modal) {
        const modal = new tailwind.Modal(document.getElementById('fullscreen-grade-modal'));
        modal.show();
    } else {
        // Fallback: show modal directly
        document.getElementById('fullscreen-grade-modal').style.display = 'block';
    }
}

// Event listeners for grade photo functionality
document.addEventListener('DOMContentLoaded', function() {
    // Fullscreen button
    const fullscreenBtn = document.getElementById('fullscreen-grade-btn');
    if (fullscreenBtn) {
    ...
    const gradePhotoPreview = document.getElementById('grade-photo-preview');
    if (gradePhotoPreview) {
        gradePhotoPreview.addEventListener('click', function(e) {
            e.preventDefault();
            toggleFullscreen();
        });
    }
