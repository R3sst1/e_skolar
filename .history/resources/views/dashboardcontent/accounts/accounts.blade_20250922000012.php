                    ...
                    <tr class="border-b border-slate-100 {{ $loop->even ? 'bg-slate-50' : 'bg-white' }}">
                        ...
                        <td class="border-b border-slate-100 py-3 text-center">
                            <div class="flex justify-center items-center gap-1 flex-wrap">
                                    ...
                                    @if($user->role === 'applicant')
                                        <div class="flex gap-1">
                                            <form method="POST" action="{{ route('accounts.promote', $user->id) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm bg-green-500 hover:bg-green-600 text-white px-2 py-1 rounded text-xs"
                                                        onclick="return confirm('Are you sure you want to promote this user to Admin?')">
                                                    <i data-lucide="arrow-up" class="w-3 h-3 mr-1"></i> Admin
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('accounts.promote-super', $user->id) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm bg-purple-500 hover:bg-purple-600 text-white px-2 py-1 rounded text-xs"
                                                        onclick="return confirm('Are you sure you want to promote this user to Super Admin? This gives them full system access.')">
                                                    <i data-lucide="crown" class="w-3 h-3 mr-1"></i> Super
                                                </button>
                                            </form>
                                        </div>
                                    @elseif($user->role === 'admin')
                                        <div class="flex gap-1">
                                            <form method="POST" action="{{ route('accounts.demote', $user->id) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm bg-orange-500 hover:bg-orange-600 text-white px-2 py-1 rounded text-xs"
                                                        onclick="return confirm('Are you sure you want to demote this user to Applicant?')">
                                                    <i data-lucide="arrow-down" class="w-3 h-3 mr-1"></i> Demote
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('accounts.promote-super', $user->id) }}" class="inline">
                                                @csrf
                                ...
                                <!-- Edit Button -->
                                <button class="btn btn-sm bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs" 
                                ...
                                <form method="POST" action="{{ route('accounts.destroy', $user->id) }}" class="inline" 
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded text-xs"
                                ...
                                <a href="{{ route('view.profile.other', $user->id) }}" 
                                   class="btn btn-sm bg-slate-500 hover:bg-slate-600 text-white px-2 py-1 rounded text-xs">
