...
@section('content')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const scholarStatusCtx = document.getElementById('scholarStatusChart');
        if (scholarStatusCtx) {
            new Chart(scholarStatusCtx.getContext('2d'), {
                type: 'doughnut',
                data: {
        ...
        const monthlyDisbursementCtx = document.getElementById('monthlyDisbursementChart');
        if (monthlyDisbursementCtx) {
            new Chart(monthlyDisbursementCtx.getContext('2d'), {
                type: 'line',
                data: {
        ...
        const retentionTrendCtx = document.getElementById('retentionTrendChart');
        if (retentionTrendCtx) {
            new Chart(retentionTrendCtx.getContext('2d'), {
                type: 'radar',
                data: {
                    ...
                    datasets: [{
                        ...
                        data: [
                            {{ \App\Models\Scholar::where('status', 'active')->count() }},
                            {{ \App\Models\Scholar::where('status', 'graduated')->count() }},
                            {{ \App\Models\Scholar::count() > 0 ? ((\App\Models\Scholar::where('status', 'active')->count() + \App\Models\Scholar::where('status', 'graduated')->count()) / \App\Models\Scholar::count()) * 100 : 0 }},
                            {{ \App\Models\Scholar::count() > 0 ? ((\App\Models\Scholar::where('status', 'active')->count() + \App\Models\Scholar::where('status', 'graduated')->count()) / \App\Models\Scholar::count()) * 100 : 0 }},
                            {{ \App\Models\Scholar::count() > 0 ? ((\App\Models\Scholar::where('status', 'active')->count() + \App\Models\Scholar::where('status', 'graduated')->count()) / \App\Models\Scholar::count()) * 100 : 0 }}
                        ],
