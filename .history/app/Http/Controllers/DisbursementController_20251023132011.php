...
class DisbursementController extends Controller
{
    ...
    public function approve($id)
    {
        ...
        $disbursementBatch = DisbursementBatch::findOrFail($id);
        
        if ($disbursementBatch->status !== 'pending') {
            return redirect()->back()->with('error', 'Only pending batches can be approved.');
        }

        $disbursementBatch->update(['status' => 'reviewed']);

        return redirect()->back()->with('success', 'Disbursement batch approved successfully!');
    }
    ...
    public function release($id)
    {
        ...
        $disbursementBatch = DisbursementBatch::with('scholarshipProgram')->findOrFail($id);
        
        if ($disbursementBatch->status !== 'reviewed') {
            return redirect()->back()->with('error', 'Only reviewed batches can be released.');
        }

        // Budget guard: ensure enough remaining allocated budget for this program
        $program = $disbursementBatch->scholarshipProgram;
        $allocated = (float) ($program->allocated_budget ?? 0);
        $alreadyDisbursed = (float) DisbursementBatch::where('scholarship_program_id', $program->id)
            ->whereIn('status', ['reviewed', 'disbursed'])
            ->sum('total_amount');
        $remaining = max(0, $allocated - $alreadyDisbursed);
        $needed = (float) ($disbursementBatch->total_amount ?? 0);
        if ($program->type === 'budgeted' && $allocated > 0 && $needed > $remaining) {
            $lack = $needed - $remaining;
            return redirect()->back()->with('error', 'Budget allocated not enough. Remaining: ₱' . number_format($remaining, 2) . ' | Required: ₱' . number_format($needed, 2) . '. Please submit an Additional Fund Request for ₱' . number_format($lack, 2) . '.');
        }

        DB::beginTransaction();
        try {
            $disbursementBatch->update(['status' => 'disbursed']);
            
            // Update all students in the batch to disbursed status
            $disbursementBatch->disbursementBatchStudents()->update(['status' => 'disbursed']);

            DB::commit();

            return redirect()->back()->with('success', 'Disbursement batch released successfully!');

        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->back()->with('error', 'Failed to release disbursement batch: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Add initial allocation data for testing purposes
     * This method can be called to set up initial budget allocation
     */
    public function addInitialAllocation(Request $request)
    {
        ...
        try {
            AllocationLog::create([
                ...
                'description' => $request->description ?? 'Initial budget allocation from E-Kalinga system',
                'reference_number' => 'ALLOC-' . now()->format('YmdHis'),
            ]);

            return redirect()->back()->with('success', 'Initial allocation of ₱' . number_format($request->amount, 2) . ' added successfully.');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Failed to add allocation: ' . $e->getMessage());
        }
    }
