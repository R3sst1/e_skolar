...
class ApplicationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'requirements.report_card' => 'required|file|mimes:pdf,jpg,jpeg,png|max:2048',
            'requirements.registration' => 'required|file|mimes:pdf,jpg,jpeg,png|max:2048',
            'requirements.good_moral' => 'required|file|mimes:pdf,jpg,jpeg,png|max:2048',
            'requirements.barangay_cert' => 'required|file|mimes:pdf,jpg,jpeg,png|max:2048',
            'requirements.income_cert' => 'required|file|mimes:pdf,jpg,jpeg,png|max:2048',
        ]);
        ...
        try {
            ...
            // Store requirements
            foreach ($request->file('requirements') as $type => $file) {
                $path = $file->store('requirements/' . $application->id, 'public');
    ...
    public function approve($id)
    {
        ...
        $currentScholarsBarangay = \App\Models\Scholar::where('barangay', $application->user->barangay)
            ->where('status', 'active')
            ->count();
        if ($currentScholarsBarangay >= $maxScholarsBarangay) {
            $message = 'Cannot approve: Maximum number of scholars for this barangay ('.$application->user->barangay.') reached.';
            if (request()->expectsJson()) {
                return response()->json(['success' => false, 'error' => $message], 422);
            }
            return back()->with('error', $message);
        ...
        try {
            ...
            if (empty($application->user->barangay)) {
                $message = 'Cannot approve: User barangay information is missing. Please update the user profile first.';
                \Log::warning('User barangay missing', ['user_id' => $application->user_id]);
                if (request()->expectsJson()) {
                    return response()->json(['success' => false, 'error' => $message], 422);
                }
                return back()->with('error', $message);
            ...
            \Log::info('Creating scholar record', [
                ...
                'barangay' => $application->user->barangay,
    ...
    public function applicants(Request $request)
    {
        ...
        return view('dashboardcontent.applicants', compact('applications'));
    }
}
